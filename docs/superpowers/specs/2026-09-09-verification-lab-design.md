...
Keep the original wallet dashboard at /archive/base-sepolia, with wallet providers loaded only there. Preserve its historical Base Sepolia evidence and local setup. Its baseline settlement is payment-only and must be explicitly opted into; it must not appear equivalent to quality-gated settlement. The public server-funded buyer route is disabled in production. Solana devnet is the sole active and future testnet target. /testnet explains its integration status; no live Solana implementation is claimed.
